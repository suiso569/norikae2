...
# ==========================================
# 方面を自動判定するための、各路線の正しい駅順リスト（端から端）
LINES_STATIONS = {
...
def get_direction(line_name, from_station, to_station):
    if line_name in LINES_STATIONS:
        ...
        if from_station in stations and to_station in stations:
            ...
            # 駅順リストのインデックスが進む方向なら末尾の駅、戻る方向なら先頭の駅
            if idx_to > idx_from:
...
def build_routing_graph():
    ...
    for line_name, edges in lines.items():
        for u, v, w in edges:
            ...
            # 快速優先ロジック: 快速のコストを微小(-0.1秒)に削り、同タイムなら快速を選ばせる
...
# ==========================================
# 4. 動的路線図の生成ロジック (Graphviz)
# ==========================================
def generate_visual_map(path_stations=None):
    dot = graphviz.Graph(engine='dot')
    dot.attr(rankdir='LR', size='11,7', dpi='150')
    dot.attr('node', shape='box', style='rounded,filled', fillcolor='#FFFFFF', color='#333333', fontname='sans-serif')
    ...
    for station in sorted(all_stations):
        if station in path_nodes:
            dot.node(station, station, fillcolor='#FFFEE0', color='#FFD700', penwidth='2.5')
        else:
    ...
    for line_name, edges in lines.items():
        ...
        for u, v, _ in edges:
            ...
            if (edge_key, line_name) not in added_edges:
                ...
                if path_stations: # 検索モード時
                    if is_on_path:
                        dot.edge(u, v, color=base_color, penwidth='3.5', label=line_name)
                    else:
                        # 経路外は透明度「25」を付与して半透明化
                        dot.edge(u, v, color=f"{base_color}25", penwidth='1.0', style='dashed' if "快速" in line_name else 'solid')
                else: # 通常モード時
                    dot.edge(u, v, color=base_color, penwidth='1.8', style='dashed' if "快速" in line_name else 'solid')
    ...
    for u, v in walks:
        ...
        if path_stations:
            if is_on_path:
                dot.edge(u, v, color="#666666", penwidth='3.5', style='dotted', label="徒歩")
            else:
                dot.edge(u, v, color="#66666625", penwidth='1.0', style='dotted')
        else:
            dot.edge(u, v, color="#666666", penwidth='1.5', style='dotted')
...
def main():
    ...
    if st.button("経路を検索", type="primary", use_container_width=True):
        ...
        try:
            ...
            # 経路表示の下側にハイライト路線図を掲載
            st.divider()
            st.markdown("### 🗺️ 運行ルート路線図 (赤枠が今回の経路)")
            route_map = generate_visual_map(path_stations)
            st.graphviz_chart(route_map)
    ...
    # 画面最下部に全体のネットワーク図を常時掲載
    st.markdown("#### 🌐 全体路線ネットワーク図")
    st.graphviz_chart(generate_visual_map())
